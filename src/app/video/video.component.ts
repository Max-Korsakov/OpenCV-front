...
export class VideoComponent implements AfterViewInit {
  ...
  public singUpState = false;
  public timer;
  public enhense = false;

  ngAfterViewInit() {
    // this.video = document.getElementById("video");
    // Module._main = this.opencvIsReady();
    // setTimeout(()=>console.dir(Module), 1000)
  }

  public signUp() {
    if (this.email && this.password) {
      this.singUpState = true;
      if (this.enhense) {
        setTimeout(() => {
          this.video = document.getElementById("video");
          Module._main = this.opencvIsReady();
  ...
  public processVideo() {
    ...
    //this.srcMat.data.set(this.video);
    ...
    this.isDetected = !!faces.length;
    if (!this.isDetected && !this.timer) {
    ...
    } else if (this.isDetected && this.timer) {
    ...
    // if (container.childNodes.length < 1500) {
    //   requestAnimationFrame(this.processVideo.bind(this));
    // } else {
    //   this.videoShow = true;
  ...
  public stopVideoProcessing() {
    if (this.src != null && !this.src.isDeleted()) {
      this.src.delete();
    }
    if (this.dstC1 != null && !this.dstC1.isDeleted()) {
      this.dstC1.delete();
    }
    if (this.dstC3 != null && !this.dstC3.isDeleted()) {
      this.dstC3.delete();
    }
    if (this.dstC4 != null && !this.dstC4.isDeleted()) {
      this.dstC4.delete();
    }
  }
